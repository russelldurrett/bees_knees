...
'''
usage: python sequence_shuffler.py sample_list.txt, filetype, velvetdirectory, output_dir
'''
...
for sample in names: 
	...
	print 'found these files: ' + ' '.join(sample_fastqs)
	...
	args_str = sys.argv[3] + '/contrib/shuffleSequences_fasta/shuffleSequnces_' + sys.argv[2]+ '.pl ' + ' '.join(sample_fastqs) + ' ' + sys.argv[4] + '/' + str(sample_name) + '_interleaved.fastq'
	print args_str
